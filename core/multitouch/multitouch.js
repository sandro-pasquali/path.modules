...
var hammertime = Hammer(this.$view.get(0));

this.api = {
	on	: function(gesture, selector, handler) {
		if(typeof selector === 'function') {
			handler 	= selector;
			selector 	= false;
		}
		hammertime.on(gesture, function(event) {
			if(selector && !jQuery(event.target).is(selector)) {
				return;
			}
			handler(event);
		});
...
this.api.on('tap', '.donttouchme', function() {
	console.log('tap');
