...
SlipHover.prototype = {
	...
	_ini: function(element, options) { //the underscore indicates this is a private method
		var that = this,
			...
			$targets = $element.find('img').size() > 0 ? $element.find('img') : $element;
...
this.$view.find('img[sliphover-text]').each(function() {

	var $this = $(this);
	
	var entry = new SlipHover($this, {
		flip			: $this.attr('sliphover-flip') === "true" ? true : false,
		duration		: $this.attr('sliphover-duration') || "fast",
		fontColor		: $this.attr('sliphover-fontColor') || "white",
		backgroundColor	: $this.attr('sliphover-backgroundColor') || "rgba(0,0,0,.5)",
		reverse			: $this.attr('sliphover-reverse') === "true" ? true : false,
		autoScroll		: $this.attr('sliphover-autoScroll') === "false" ? false : true,
		scrollSpeed		: $this.attr('sliphover-scrollSpeed') || 40,
		height			: $this.attr('sliphover-height') || '100%'
	});
	
	entry._ini();

	//handle the window resize event for it will destroy the layout
	$(window).off('resize.sliphover').on('resize.sliphover', function() {
